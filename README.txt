...
We compute a hash value for each possible pattarn- 
GIven a line and an index of a changed word i, we compute the hash value of the concatination of the rest of the line's words h(x_i). 
...
Because the computation of the hash is independant for each line, we could compute it simultaneously.
The threads would fill the same hashMap, hence we need to use locks.
